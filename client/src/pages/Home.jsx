...
/* ─── UPGRADED FILTER BAR ─────────────────────────────────────── */
const FilterBar = memo(function FilterBar({ activeFilter, setActiveFilter, filterButtons }) {
  const barRef = useRef(null);
  const [hlStyle, setHlStyle] = useState({ left: 0, width: 0 });
  ...
  useEffect(() => {
    ...
    if (active) {
      setHlStyle({ left: active.offsetLeft, width: active.offsetWidth });
    }
  }, [activeFilter]);
  ...
  const handleClick = (key, e) => {
    ...
    setHlStyle({ left: btn.offsetLeft, width: btn.offsetWidth });
  };
...
/* ─── UPGRADED SIZE SELECTOR ──────────────────────────────────── */
const SizeSelector = memo(function SizeSelector({ jersey, selectedSize, setSelectedSize, onAddToCart }) {
  ...
  const stockPipClass = (size) => {
    if (getSizeStock(size) === 0) return "stock-pip";
    if (size === selectedSize) return "stock-pip sel";
    if (getSizeStock(size) <= 2) return "stock-pip low";
    return "stock-pip ok";
  };
  ...
  return (
    <div style={{ padding: "0 24px 32px" }}>
      ...
      {/* Size grid */}
      <div className={`size-grid${selectedSize ? " has-selection" : ""}`}>
        {sizes.map(s => {
          ...
          return (
            <button
              key={s}
              className={`size-btn${isSelected ? " selected" : ""}${oos ? " oos" : ""}`}
      ...
      <div className="size-stock-row">
        {sizes.map(s => {
          ...
          const isLow = !oos && stock <= 2;
          return (
            <div
              key={s}
              className={`stock-pip${oos ? "" : isSelected ? " sel" : isLow ? " low" : " ok"}`}
            />
          );
        })}
      </div>
      ...
      {/* Low stock warning */}
      {selectedSize && getSizeStock(selectedSize) <= 2 && getSizeStock(selectedSize) > 0 && (
        <div style={{ fontSize: 11, color: "#ff6b00", letterSpacing: 3, marginTop: 10, fontWeight: 900 }}>
          ⚠ ONLY {getSizeStock(selectedSize)} LEFT IN THIS SIZE
        </div>
      )}
...
export default function JerseyStore() {
  ...
  const addToCart = useCallback((jersey, size) => {
    ...
    showToast(`${jersey.name} (${size}) added to cart!`);
  ...
  const handleLogout = useCallback(async () => {
    ...
    setUser(null); setIsAdmin(false); setCart([]);
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  #jv-root .add-btn, #jv-root .checkout-btn, #jv-root .btn-add { display: block; width: 100%; text-align: center; }
  ...
  @keyframes filterSweep { 0%{transform:translateX(-100%);opacity:0;} 30%{opacity:1;} 60%{transform:translateX(200%);opacity:0;} 100%{transform:translateX(200%);opacity:0;} }
  @keyframes sizeFlash  { 0%{opacity:1;} 100%{opacity:0;} }
  ...
  /* ── CARD "SELECT SIZE" BUTTON ── */
  #jv-root .add-btn {
    position:relative; overflow:hidden;
    background:#39ff14; color:#000; border:none !important;
  ...
  #jv-root .add-btn:hover { background:#000 !important; color:#39ff14 !important; box-shadow:inset 0 0 0 2px #39ff14; letter-spacing:8px; }
  #jv-root .add-btn:active { transform:scale(0.98); }
  #jv-root .add-btn:disabled,#jv-root .add-btn[disabled] { background:#1a1a1a !important; color:#333 !important; cursor:not-allowed; letter-spacing:3px; box-shadow:none !important; }
  ...
  .filter-bar {
    ...
    background:#080808; border:1.5px solid #1c1c1c;
  ...
  .filter-highlight::after {
    ...
    animation:filterSweep 2.4s ease-in-out infinite;
  }
  ...
  .filter-btn {
    ...
    padding:12px 0; flex:1; text-align:center;
    color:#232323;
    transition:color 0.18s;
    white-space:nowrap; border-radius:2px;
  }
  .filter-btn:not(.active):hover { color:#555; }
  .filter-btn.active { color:#000; font-weight:900; }
  /* Dim non-active when something is selected */
  .filter-bar .filter-btn:not(.active) { color:#1e1e1e; }
  .filter-bar .filter-btn:not(.active):hover { color:#555; }
  ...
  .size-label-text {
    ...
    color:#2a2a2a; font-weight:900; text-transform:uppercase; flex-shrink:0;
  }
  .size-label-row::after {
  ...
  .size-btn {
    ...
    /* Default: very dark / nearly invisible */
    background:#0c0c0c; border:1.5px solid #141414; color:#1e1e1e;
  }

  /* When grid has a selection, crush non-selected further */
  .size-grid.has-selection .size-btn:not(.selected):not(.oos) {
    background:#080808; border-color:#0f0f0f; color:#141414;
    filter:brightness(0.55);
  }

  .size-btn:not(.selected):not(.oos):hover {
    background:#071207; border-color:#39ff14; color:#39ff14;
    transform:translateY(-5px);
    box-shadow:0 12px 32px rgba(57,255,20,0.18), 0 0 0 1px #39ff14;
    filter:brightness(1) !important;
    border-radius:8px;
  }

  /* Selected: BLASTING neon */
  .size-btn.selected {
  ...
  .size-btn.selected::after {
    ...
    animation:sizeFlash 0.35s ease forwards;
  }

  /* OOS: crossed out */
  .size-btn.oos {
    background:#060606; border-color:#0d0d0d; color:#111; cursor:not-allowed;
  }
  .size-btn.oos::before, .size-btn.oos::after {
    content:''; position:absolute; width:55%; height:1px; background:#1c1c1c;
  }
  .size-btn.oos::before { transform:rotate(40deg); }
  .size-btn.oos::after  { transform:rotate(-40deg); }

  /* Stock pip strip */
  .size-stock-row { display:flex; gap:6px; margin-top:8px; }
  .stock-pip { flex:1; height:2px; background:#141414; border-radius:1px; transition:background 0.2s; }
  .stock-pip.ok  { background:#1e3a14; }
  .stock-pip.low { background:#6b3000; }
  .stock-pip.sel { background:#39ff14; box-shadow:0 0 8px #39ff14; }
  ...
  #jv-root .btn-add {
    ...
    padding:0 26px; height:70px; width:100%;
    background:#39ff14; color:#000;
    font-family:'Bebas Neue',sans-serif;
    font-size:20px; letter-spacing:7px; text-transform:uppercase;
    transition:letter-spacing 0.25s cubic-bezier(0.23,1,0.32,1), box-shadow 0.25s;
    box-shadow:0 0 40px rgba(57,255,20,0.35), 0 0 80px rgba(57,255,20,0.1);
  }
  #jv-root .btn-add::before {
    content:''; position:absolute; inset:0;
    background:#000; transform:scaleX(0); transform-origin:left;
    transition:transform 0.35s cubic-bezier(0.23,1,0.32,1);
  }
  #jv-root .btn-add:hover::before { transform:scaleX(1); }
  #jv-root .btn-add:hover {
    letter-spacing:10px;
    box-shadow:inset 0 0 0 2px #39ff14, 0 0 60px rgba(57,255,20,0.2);
  }
  #jv-root .btn-add:active { transform:scale(0.99); }
  #jv-root .btn-add .btn-label, #jv-root .btn-add .btn-price {
    position:relative; z-index:1; transition:color 0.25s;
  }
  #jv-root .btn-add:hover .btn-label,
  #jv-root .btn-add:hover .btn-price { color:#39ff14; }
  #jv-root .btn-add .btn-price { font-size:24px; font-style:italic; letter-spacing:2px; }

  /* Corner bracket accents */
  #jv-root .btn-add .c {
    position:absolute; width:10px; height:10px;
    border-color:rgba(0,0,0,0.22); border-style:solid; z-index:2;
    transition:border-color 0.25s;
  }
  #jv-root .btn-add:hover .c { border-color:rgba(57,255,20,0.45); }
  #jv-root .btn-add .c.tl { top:7px;    left:7px;   border-width:1.5px 0 0 1.5px; }
  #jv-root .btn-add .c.tr { top:7px;    right:7px;  border-width:1.5px 1.5px 0 0; }
  #jv-root .btn-add .c.bl { bottom:7px; left:7px;   border-width:0 0 1.5px 1.5px; }
  #jv-root .btn-add .c.br { bottom:7px; right:7px;  border-width:0 1.5px 1.5px 0; }
  ...
  @media(max-width:768px) {
    ...
    .hero-section { padding:60px 16px 40px; background-position:center center; }
    ...
    .size-btn { height:58px; font-size:18px; }
    .filter-btn { font-size:13px; letter-spacing:3px; padding:11px 0; }
  }
  @media(max-width:480px) {
    .stat-cell { padding:14px 0; }
    .size-grid { gap:4px; }
    .size-btn { height:52px; font-size:16px; }
  }
`}</style>
        ...
        <nav style={{ position:"sticky", top:0, zIndex:50, background:"rgba(10,10,10,0.97)", backdropFilter:"blur(10px)", borderBottom:"1px solid #1a1a1a", padding:"0 20px 0 4px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:16, height:64, animation:"slideDown 0.5s ease" }}>
          ...
          <div className="nav-right" style={{ display:"flex", alignItems:"center", gap:16, flexShrink:0 }}>
            <button onClick={() => setCartOpen(true)} style={{ background:"transparent", border:"none", color:"#fff", cursor:"pointer", display:"flex", alignItems:"center", gap:8, fontFamily:"'Barlow Condensed',sans-serif", fontWeight:900, fontSize:15, letterSpacing:1, padding:0, transition:"color 0.2s" }}
              ...
              {cartCount > 0 && <span style={{ color:"#39ff14", fontSize:20, fontWeight:900, lineHeight:1 }}>{cartCount}</span>}
            </button>
          ...
          <div className={`mobile-menu${mobileMenuOpen ? " open" : ""}`}>
            <input className="search-input" placeholder="SEARCH..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ marginBottom:8 }} />
            {navLinks}
          </div>
        </nav>
        ...
        <section className="hero-section" style={{ opacity:heroVisible?1:0, transition:heroVisible?"none":"opacity 0.8s ease", backgroundImage:`url(${heroBg})` }}>
          <div style={{ position:"absolute", inset:0, background:"linear-gradient(to bottom,rgba(10,10,10,0.9) 0%,rgba(10,10,10,0.4) 30%,rgba(0,0,0,0.3) 60%,rgba(10,10,10,0.98) 100%)", pointerEvents:"none" }} />
          ...
          <div style={{ marginTop:32, display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", position:"relative", zIndex:1 }}>
            ...
            <button onClick={() => navigate("/teams")} style={{ all:"unset", boxSizing:"border-box", display:"inline-flex", alignItems:"center", justifyContent:"center", background:"transparent", color:"#fff", border:"1px solid #333", padding:"14px 36px", fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:15, letterSpacing:3, cursor:"pointer" }}>
              VIEW TEAMS
            </button>
          </div>
          ...
          {loadingProducts ? (
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:2 }}>
              {filtered.map((jersey, i) => (
                <div key={jersey.id} className="card" onClick={() => { setSelectedJersey(jersey); setSelectedSize(null); }}
                  ...
                  <div style={{ padding:"12px 16px 16px" }}>
                    <button
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        if (jersey.stock > 0) { setSelectedJersey(jersey); setSelectedSize(null); }
                      }}
                    >
        ...
        <footer style={{ background:"#050505", borderTop:"1px solid #1a1a1a", padding:"40px 24px", textAlign:"center" }}>
          ...
          <p style={{ color:"#333", fontSize:12, letterSpacing:2 }}>©️ 2026 JERSEYVAULT. ALL RIGHTS RESERVED.</p>
        ...
        {/* ── SIZE PICKER MODAL (UPGRADED) ── */}
        {selectedJersey && (
          <div className="modal-bg" onClick={() => { setSelectedJersey(null); setSelectedSize(null); }}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              {/* Image */}
              <div style={{ position:"relative" }}>
                <button onClick={() => { setSelectedJersey(null); setSelectedSize(null); }}
                  style={{ position:"absolute", top:12, right:12, background:"rgba(0,0,0,0.7)", border:"1px solid #333", color:"#fff", fontSize:16, cursor:"pointer", width:32, height:32, display:"flex", alignItems:"center", justifyContent:"center", zIndex:2, fontFamily:"'Barlow Condensed',sans-serif", fontWeight:900 }}>✕</button>
              ...
              {/* ── UPGRADED SIZE SELECTOR inside modal ── */}
              <SizeSelector
