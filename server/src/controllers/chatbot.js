...
function extractOrderOrTrackingId(text) {
  if (!text) return null;
  const patterns = [
    /\bTRK-[A-Z0-9]{6}\b/i,
    /\bpay_[a-zA-Z0-9]+\b/i,
...
async function fetchOrderDetails(idOrTrackingId) {
  try {
    const cleanId = idOrTrackingId.trim().toUpperCase();

    // Sign in as admin to bypass RLS restrictions if using public key
    const adminEmail = process.env.CHATBOT_ADMIN_EMAIL;
    const adminPassword = process.env.CHATBOT_ADMIN_PASSWORD;
    if (adminEmail && adminPassword) {
      try {
        await localSupabase.auth.signInWithPassword({
          email: adminEmail,
          password: adminPassword
        });
      } catch (authErr) {
        console.error('Local chatbot auth signin failed:', authErr);
      }
    }
    ...
    console.error('Database query error in local chatbot:', err);
    return null;
  }
}

function extractJerseySearchTerms(text) {
...
async function findAvailableJerseys(terms) {
...
function getLocalResponse(message, orderDetails, matchedJerseys, searchTerms) {
  const msg = message.toLowerCase();

  // 1. Order tracking details query
  if (orderDetails) {
    const statusText = statusLabels[orderDetails.status] || 'UNKNOWN';
  ...
  if (matchedJerseys !== null) {
    if (matchedJerseys.length > 0) {
      const list = matchedJerseys
        .map(j => `- **${j.name}** (${j.type === 'player' ? 'Player Version' : 'Fan Version'}) — ₹${j.price} [In Stock]`)
        .join('\n');
...
export const handleChat = async (req, res) => {
  ...
  // 1. Check for Order ID / Tracking ID in the user's message
  ...
  if (isInventoryQuery && searchTerms.length > 0 && !orderId) {
    matchedJerseys = await findAvailableJerseys(searchTerms);
  }
  ...
  // 4. Return response
  res.json({ text: responseText, orderData: orderDetails });
};
