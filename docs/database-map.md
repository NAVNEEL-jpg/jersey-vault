...
| **orders** | `id` (UUID) | `customer_name`, `customer_email`, `customer_phone`, `address`, `city`, `state`, `pincode`, `items` (JSON), `subtotal`, `shipping`, `total`, `pay_method`, `status`, `razorpay_order_id`, `razorpay_payment_id`, `payment_captured`, `created_at` | Created in `orderController` & `paymentController` (pending → confirmed). |
| **products** | `id` (UUID) | `name`, `price`, `size_stock` (JSON), `category`, `description`, `image_url`, `rating`, `reviews_count` | Read for product listings, stock decrement after orders, reviews association. |
